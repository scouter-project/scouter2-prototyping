...
//        limit = Math.max(configXlog.getXlogRealtimeLowerBoundMs(), limit);

        ListValue objHashLv = param.getList("objHash");

        MutableLongList objIds = LegacySupport.listValue2LongList(objHashLv);

        cacheManager.getCache(LegacySupport.APPLICATION_ID_FOR_SCOUTER1_AGENT)
                .getAndHandleRealTimeXLog(objIds.toSet(), loop, index, 100000, new RealtimeXlogStreamObserver() {

            @Override
            public void onLoad(RealtimeXlogOffset lastOffset) {
                try {
                    val outPack = new MapPack();
                    outPack.put("loop", new DecimalValue(lastOffset.getLoop()));
                    outPack.put("index", new DecimalValue(lastOffset.getIndex()));
